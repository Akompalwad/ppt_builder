...
class PresentationPIIVault(Base):
    """Encrypted, owner-scoped substitutions retained only until file expiry."""
    __tablename__="presentation_pii_vaults"; presentation_id: Mapped[str]=mapped_column(ForeignKey("presentations.id"),primary_key=True); user_id: Mapped[str]=mapped_column(ForeignKey("users.id")); encrypted_tokens: Mapped[str]=mapped_column(String(16000)); created_at: Mapped[datetime]=mapped_column(DateTime,default=datetime.utcnow)
