...
def render_slide_html(spec: PresentationSpec, slide_number: int) -> str:
    ...
    if slide.layout_type.value=="title_slide":
        ...
        steps="".join(f"<section class='flow-step'><div class='chevron'><span>{i+1:02d}</span><h3>{esc(item.heading or f'Step {i+1}')}</h3></div><div class='flow-detail'><small>DETAIL</small><p>{body(item)}</p></div></section>" for i,item in enumerate(slide.elements[:4]))
    ...
    return textwrap.dedent(f'''<style>
      ...
      .chevrons {{ display:grid; grid-template-columns:repeat({min(4,max(1,len(slide.elements[:4])))},1fr); gap:7px; margin-top:8%; }} .flow-step {{ display:grid; grid-template-rows:96px 1fr; gap:12px; min-width:0; }} .chevron {{ padding:18px 17px; clip-path:polygon(0 0,88% 0,100% 50%,88% 100%,0 100%,12% 50%); background:{d.surface_color}; border:1px solid {d.primary_color}; }} .flow-step:first-child .chevron {{ background:{d.primary_color}; color:{d.background_color}; }} .chevron span {{ font-weight:800; font-size:11px; }} .chevron h3 {{ margin:21px 5px 0; font-size:clamp(13px,1.25vw,18px); }} .flow-detail {{ min-height:150px; padding:15px 14px; border:1px solid {d.primary_color}; border-radius:10px; background:linear-gradient(145deg,{d.surface_color},{d.background_color}); box-shadow:6px 8px 16px rgba(0,0,0,.3); }} .flow-detail small {{ color:{d.primary_color}; font-weight:800; font-size:10px; letter-spacing:.06em; }} .flow-detail p {{ margin:13px 0 0; color:{d.text_secondary}; font-size:clamp(11px,1vw,14px); line-height:1.32; text-align:center; }}
