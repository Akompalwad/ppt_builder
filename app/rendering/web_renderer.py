...
def render_slide_html(spec: PresentationSpec, slide_number: int) -> str:
    ...
    if slide.layout_type.value=="title_slide":
        ...
        steps="".join(f"<section class='chevron'><span>{i+1:02d}</span><h3>{esc(item.heading or f'Step {i+1}')}</h3><p>{body(item)}</p></section>" for i,item in enumerate(slide.elements[:4]))
